# Быстрый старт - Система лицензирования

## Шаг 1: Добавить DATABASE_URL в Vercel

1. Перейдите в [Vercel Dashboard](https://vercel.com/dashboard)
2. Выберите ваш проект LandscapeHelper
3. Перейдите в **Settings** → **Environment Variables**
4. Добавьте новую переменную:
   - **Name**: `DATABASE_URL`
   - **Value**: `postgresql://neondb_owner:npg_3QPdCOmfDK2e@ep-silent-bar-a10loy9g-pooler.ap-southeast-1.aws.neon.tech/neondb?sslmode=require`
   - Выберите все окружения (Production, Preview, Development)
5. Нажмите **Save**

## Шаг 2: Добавить ADMIN_TOKEN

Добавьте еще одну переменную:
- **Name**: `ADMIN_TOKEN`
- **Value**: Придумайте сложный секретный токен (например: `lkJh3k9mP2qR5tW8xY1zA4bC6dE7fG`)
- Это токен для доступа к админ-панели генерации лицензий

## Шаг 3: Настроить Email

Добавьте переменные для отправки email:

- **SMTP_HOST**: `mail.227.info`
- **SMTP_PORT**: `465` (или `587` для STARTTLS)
- **SMTP_USER**: `admin@227.info`
- **SMTP_PASS**: `Gdv2210974!!!`
- **EMAIL_FROM**: `LandscapeHelper <admin@227.info>`

**Примечание**: Для порта 465 используйте `secure: true`, для 587 используйте `secure: false` (STARTTLS).

## Шаг 4: Деплой проекта

После добавления переменных окружения:

```bash
git add .
git commit -m "Add license system"
git push origin license-system
```

Или задеплойте через Vercel Dashboard.

## Шаг 5: Инициализация базы данных

После деплоя откройте в браузере:

```
https://landscape.227.info/api/init-db
```

Или выполните POST запрос:

```bash
curl -X POST https://landscape.227.info/api/init-db
```

Должен вернуться ответ:
```json
{
  "success": true,
  "message": "Database initialized successfully. Table \"licenses\" created with indexes."
}
```

## Шаг 6: Проверка подключения

Проверьте подключение к БД:

```
https://landscape.227.info/api/test-db
```

Должен вернуться ответ с информацией о БД.

## Шаг 7: Генерация первой лицензии

1. Откройте страницу `/payment` на вашем сайте
2. Нажмите "🔑 Показать админ-панель"
3. Введите ваш `ADMIN_TOKEN`
4. Выберите срок действия (6 или 12 месяцев)
5. (Опционально) Укажите email для отправки ключа
6. Нажмите "✅ Сгенерировать лицензию"

## Готово! 🎉

Теперь система лицензирования работает. Вы можете:
- Генерировать лицензионные ключи через админ-панель
- Отправлять ключи на email (если настроен SMTP)
- Активировать лицензии через API `/api/activate-license`
- Проверять статус лицензий через API `/api/license-status`

## Troubleshooting

### Ошибка подключения к БД
- Проверьте, что `DATABASE_URL` правильно добавлен в Vercel
- Убедитесь, что все окружения (Production, Preview, Development) выбраны
- Проверьте, что БД доступна из интернета

### Таблица не создается
- Проверьте логи в Vercel Dashboard → Functions
- Убедитесь, что `DATABASE_URL` правильный
- Попробуйте создать таблицу вручную через Neon SQL Editor

### Email не отправляется
- Проверьте SMTP настройки
- Для Gmail используйте "Пароль приложения", а не обычный пароль
- Проверьте логи в Vercel Dashboard

